/*
 This file is a fork of the upstream. The only changes to the files are really just to
 remove any dependencies on the FDB bindings directly to ease dependency management.
*/
...
// Key allows Key to (trivially) satisfy the KeyConvertible interface.
func (k Key) Key() Key {
	return k
}
...
func (p *packer) encodeVersionstamp(v Versionstamp) {
	...
	if isIncomplete {
		if p.versionstampPos != -1 {
			panic("Tuple can only contain one incomplete versionstamp")
		}
...
func (p *packer) encodeTuple(t Tuple, nested bool, versionstamps bool) {
	...
	for i, e := range t {
		...
		case KeyConvertible:
			p.encodeBytes(bytesCode, []byte(e.Key()))
		case string:
		...
		case UUID:
			p.encodeUUID(e)
		case Versionstamp:
			if !versionstamps && e.TransactionVersion == incompleteTransactionVersion {
...
// satisfy the KeyConvertible interface. Key will panic in the same
...
// RangeKeySelectors allows Tuple to satisfy the Range interface. The
...
// prefix).
func (t Tuple) RangeKeySelectors() (Selectable, Selectable) {
	b, e := t.FDBRangeKeys()
